...
직전 프레임의 추론 문장(CoT)을 **무비용 초안(draft, zero draft cost)** 으로 재활용하는 speculative decode를 **실제 10 Hz
...
2층 diffusion drafter를 학습했다). 우리는 **이미 산출돼 있는 직전 프레임의 출력**을 초안으로 쓴다. 학습
없음, 모델 수정 없음.

### 검증 비용을 어떻게 상쇄하나 — block-verify
...
그대로 채택(추가 비용 없는 보너스 토큰)** 한 뒤, 받아들이지 않은 초안 뒷부분에 해당하는 KV 캐시를 잘라낸다(crop).
...
- **FlashDrive 대비 차별점이 측정으로 섰다.** 그들은 drafter를 학습했지만, 우리는 10 Hz라는 시스템 구조가
  주는 **무비용 초안**으로 학습 없이 같은 효과(안정 프레임 16×)를 얻었다.

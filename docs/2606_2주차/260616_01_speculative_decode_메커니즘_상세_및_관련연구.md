...
2. **모델의 forward는 토큰 수가 늘어도 가중치를 한 번만 읽는다.** memory-bound라 1토큰을 처리하든
   25토큰을 처리하든 가중치 읽기는 동일 → **여러 토큰을 동시에 처리하는 한계 비용이 거의 0**이다.
...
> L개 예측을 통째로 만들어 준다. 그 다음 비교는 CPU에서 배열 두 개를 나란히 훑는 **비용이 무시할 만한
> 연산**이다.
...
→ 앞의 3개(`Keep distance to`)만 받아들이고, 4번 자리엔 **모델 자신이 고른 `the`를 추가 비용 없이 보너스로 채택**한

...
**예측 초과 발견**: P5 단독 예측은 −22.6 GB(마이크로벤치 629MB × 36)였는데 실측은 −83.9 GB.
커널 수 감소가 예측과 정확히 일치하므로 측정 오류가 아니다. 해석: 68MB급 중간텐서 3개/layer가
사라지면서 **L2 32MB의 경합이 풀려 주변 커널(attention, 다른 GEMM)의 hit rate가 연쇄 개선** —
마이크로벤치(경합 없음)에서는 보이지 않는 시스템 효과. "fusion의 이득은 캐시 경합 하에서
복리로 작용한다"는 것 자체가 보고 가치가 있는 관찰이며, iGPU(작은 L2 + 공유 버스)에서
더 크게 나타날 것으로 추정된다 (후속 검증: per-kernel L2 hit 비교).

...
**예측 초과 발견** (−22.6 GB 예측 vs −83.9 GB 실측):
※ 초기 "L2 연쇄 효과" 가설은 per-kernel class 분석으로 **기각** (untouched class 변화 0.00 GB).
실제 출처는 **eager cuBLAS gate/up GEMM의 병적 DRAM 비효율** — GEMM 1개당 1.21 GB
(이론 183 MB의 6.6×, L2 hit 63%), fused 커널은 동일 수학을 5.2× 적게 (L2 hit 94%).
−70 GB가 GEMM 내부 재읽기 제거, −13.7 GB만 중간텐서 제거.
상세 분석과 후속 작업은 전용 repo로 이동: **github.com/soonhong99/umic** (private),
`results/260610_m1_prefill/260610_l2_cascade_findings.md` 참조. 이 문서는 연구 일지로 보존.
